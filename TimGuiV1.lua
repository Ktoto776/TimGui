...
local TimGui = GetClassMetatable("TimGui",TimGuiRaw,TimGuiReadOnly,true,nil,function(key, _, new)
    if key=="Opened" then
...
Header:GetPropertyChangedEvent("FirstName"):Connect(function()
    FirstName.Text = Header.FirstName
end) Header.FirstName = tostring(TimGui.SetupData["FirstHeaderName"] or "Tim")
Header:GetPropertyChangedEvent("SecondName"):Connect(function()
    TwoName.Text = Header.SecondName
end) Header.SecondName = tostring(TimGui.SetupData["SecondHeaderName"] or "Gui")
...
local PositionsBind = Classes:CreateBind()
Binder.PositionsBind = PositionsBind
Binder:SetReadOnly("PositionsBind")
...
local function MakeGUIArchitectureClass(raw:{any?}?)
    ...
    end local SpecialPositionsBind = Classes:CreateBind()
    SpecialPositionsBind:Bind(function(...)
        return PositionsBind:Run(...)
    end) local lastRefreshed,waitRefresh = false,false
    GUIArchitecture.GroupSize = UDim2.new(0,0,0,0)
    local onRefreshed = Instance.new("BindableEvent")
    GUIArchitecture.Refreshed = onRefreshed.Event
    function GUIArchitecture:Refresh(FromObject:any?)
        if waitRefresh then 
            ...
            if waitRefresh~=FromObject then return end
            waitRefresh = false
        ...
        end) local pos = UDim2.new(0,0,0,0)
        for _,v in children do
            if FromObject then
                if v.Position<FromObject.Position then 
                    continue
                end
            end pos = SpecialPositionsBind:Run(v,pos)
        end GUIArchitecture.GroupSize = pos
        if GUIArchitecture:IsA("TGuiObject") then
            GUIArchitecture:RefreshPosition()
        end onRefreshed:Fire()
        RunService.PreRender:Wait()
        lastRefreshed = false
    end SpecialPositionsBind.OnBinded:Connect(function()
        GUIArchitecture:Refresh()
    end) PositionsBind.OnBinded:Connect(function()
        GUIArchitecture:Refresh()
    end) SizeBind.OnBinded:Connect(function()
        GUIArchitecture:Refresh()
    end) GUIArchitecture.ChildRemoved:Connect(function(child)
        GUIArchitecture:Refresh(child)
    end) fullyRefresh.Event:Connect(function()
        GUIArchitecture:Refresh()
    end) GUIArchitecture.SpecialPositionsBind = SpecialPositionsBind
    return GUIArchitecture
end PositionsBind:Bind(function(TGuiObject,pos:UDim2)
    if not TGuiObject then logger:critical_error("TGuiObject is incorrect. Expected: TGuiObject") end
    ...
    if Frame and Frame.Parent then
        ...
        return pos+Frame.Size --Сделать если не вмещается в бок то в низ
    end
end) ButtonsSFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
    fullyRefresh:Fire()
end)
...
local function TGuiObjectClass(Name:string,Title:string|{string}?,Parent:any?,Object:any?)
    ...
    Object.ParentIsGlobal = true
    Object.Indent = UDim2.new(0,0,0,0)
    local oldParent
    local onAncestorChangedEvent = Instance.new("BindableEvent")
    Object.AncestorChanged = onAncestorChangedEvent.Event
    Object:SetReadOnly("AncestorChanged")
    onAncestorChanged[Object] = function()
        onAncestorChangedEvent:Fire()
    end Object.Id = currentId
    ...
    end function Object:RefreshPosition()
    ...
    Object:GetPropertyChangedEvent("Parent"):Connect(function()
        if destroyed then return end
        if Object:IsA("GUIArchitecture") then
            if Object.Parent==Object then
                Object.Parent = nil
                logger:error("Group'"..Object.Name.."'.Parent is incorrect! Changing to nil.")
                return
            end
        end if oldParent then
            onDescendantChangedListeners[oldParent](Object)
        end if Object.Parent then
            local f if onNewChildListeners[Object.Parent] then
                oldParent = Object.Parent
                f = onNewChildListeners[Object.Parent]
                Object.ParentIsGlobal = Object.Parent==TimGui.Groups
            else Object.ParentIsGlobal = true
                Object.Position = 0
            end if Object.ParentIsGlobal then
                if Object.Parent==TimGui.Groups and Object:IsA("GUIArchitecture") then
                    Object.Frame.Parent = GroupsSFrame
                else Object.Frame.Parent = nil
                end 
            else Object.Frame.Parent = ButtonsSFrame
            end f(Object)
        else Object.ParentIsGlobal = true
        end if Object:IsA("GUIArchitecture") then
            Object:RefreshSize()
            for _,child in Object:GetChildren() do
                if onAncestorChanged[child] then
                    task.spawn(onAncestorChanged[child])
                end
            end
        end onAncestorChangedEvent:Fire()
    ...
    end local SpecialColors = Classes:CreatePreset(true)
    ...
    print(Frame and Frame.Parent and Frame.Parent:IsA("GuiMain"))
    if Frame and Frame.Parent and Frame.Parent:IsA("GuiMain") then
        print(11)
        Frame.Size = UDim2.new(GuiSize.ButtonSize.X,UDim.new(0,GuiSize.ButtonSize.Y.Offset+GuiSize.ButtonSize.Y.Scale*Frame.Parent.AbsoluteSize.Y))
        return true
    end
end)
...
GuiObjects:CreateGroup("11")
task.wait(1)
local s,Settings = pcall(function()
    local Settings = TimGui.Groups:CreateGroup("Settings","Settings")
    TimGui.GlobalOpenedGroup = Settings
    Settings.Opened = true
    Settings.Title.ru = "Настройки"
    Settings.Title.uk = "Налаштування"
